(function (window, document) {
  ...
  function scheduleRegistrasiFormPrefetch(options) {
    ...
    var prefetchCfg = config.REGISTRASI_PREFETCH || {};

    // 3D-R2: dashboard harus bersih. Prefetch form registrasi tidak berjalan otomatis
    // dari dashboard, karena memicu getRegistrasiFormDefinition/getTimRef/getWilayahRef
    // sebelum kader membuka menu Registrasi. Aktifkan hanya secara eksplisit untuk debug.
    var allowPrefetch = opts.force === true ||
      prefetchCfg.ENABLE_DASHBOARD_IDLE_PREFETCH === true ||
      config.ENABLE_DASHBOARD_REGISTRASI_PREFETCH === true;

    if (!allowPrefetch) {
      return { ok: false, skipped: true, reason: 'dashboard_prefetch_disabled_3D_R2' };
    }

    var delay = typeof opts.delayMs === 'number' ? opts.delayMs : 2400;
    if (registrasiPrefetchStarted && opts.force !== true) return { ok: false, skipped: true, reason: 'prefetch_already_started' };
  ...
  function init() {
    ...
    // 3D-R2: jangan prefetch registrasi dari dashboard. Form/ref baru dimuat saat user membuka Registrasi.
  }
