(function (window, document) {
  ...
  function statusClass(status) {
    ...
    if (s === 'RED' || s === 'ERROR' || s === 'FAILED') return 'sa-status-red';
    ...
    target.innerHTML = [
      '<div id="', ROOT_ID, '" class="super-admin-page">',
        '<style>',
          ...
          '.sa-controls{display:flex;justify-content:space-between;gap:10px;align-items:center;margin:8px 0 12px;flex-wrap:wrap}.sa-time-filter{display:flex;gap:8px;align-items:center;background:#fff;border:1px solid var(--sa-line);border-radius:16px;padding:9px 11px;box-shadow:0 6px 18px rgba(15,23,42,.04)}.sa-time-filter label{font-weight:800}.sa-time-filter select{border:1px solid #cbd5e1;border-radius:12px;padding:8px;background:#fff}',
          ...
          '.sa-layout{display:grid;grid-template-columns:1.1fr .72fr .88fr;gap:12px;margin:12px 0}.sa-workspace{display:grid;grid-template-columns:minmax(0,1fr) 390px;gap:12px;align-items:start}.sa-panel{background:#fff;border:1px solid var(--sa-line);border-radius:20px;padding:14px;box-shadow:0 10px 28px rgba(15,23,42,.05)}.sa-inspector{position:sticky;top:12px}',
        ...
        '<section class="sa-controls">',
          '<div class="sa-time-filter"><label for="sa-time-window">Waktu Monitoring</label><select id="sa-time-window">',
            '<option value="today">Hari ini</option><option value="1h">1 jam terakhir</option><option value="3h">3 jam terakhir</option><option value="6h">6 jam terakhir</option><option value="24h">24 jam terakhir</option><option value="7d">7 hari terakhir</option><option value="all">Semua sampel</option>',
          '</select></div>',
          '<p class="sa-muted" id="sa-period-label">Periode: Hari ini</p>',
        '</section>',
        ...
        '<div class="sa-workspace">',
          ...
          '<aside id="sa-detail-panel" class="sa-panel sa-inspector"><h3>Detail</h3><p class="sa-muted">Klik salah satu baris tabel untuk melihat info lengkap, request ID, user/perangkat, dan rekomendasi awal.</p></aside>',
        '</div>',
      '</div>'
    ].join('');
  }
  ...
  function renderSummary(data) {
    ...
    root.innerHTML = [
      '<section class="sa-hero"><div class="sa-hero-grid"><div>',
        ...
        '<p>Periode: ', escapeHtml(periodLabel), ' · Update: ', escapeHtml(fmt(data.generated_at)), ' · Service: ', escapeHtml(fmt(data.service_version)), ' · Ambang lambat: ', escapeHtml(fmt(data.slow_threshold_ms)), ' ms · Cache: ', escapeHtml(data.cache_hit ? 'HIT' : 'MISS'), '</p>',
  ...
  function tableHtml(headers, rows, detailType) {
    ...
    return [
      ...
      rows.length ? rows.map(function (row, idx) {
        return '<tr ' + (detailType ? 'data-sa-row="' + idx + '"' : '') + '>' + headers.map(function (h) {
  ...
  function renderLogs(data) {
    ...
    if (keys.length) bindDetailRows(items, 'log');
  ...
  function showInspector(type, row) {
    ...
    if (!panel) return;
    row = row || {};
    var keys = Object.keys(row).slice(0, 24);
    panel.innerHTML = [
      '<h3>Detail</h3>',
      '<p class="sa-muted">Jenis: ', escapeHtml(humanLabel(type || 'row')), '</p>',
      '<div class="sa-detail-grid">',
        keys.map(function (k) {
          var val = row[k];
          if (k.toLowerCase().indexOf('detail') >= 0 || k.toLowerCase().indexOf('payload') >= 0) val = humanText(val);
