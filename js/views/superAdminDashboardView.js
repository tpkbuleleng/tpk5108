(function (window, document) {
  ...
  var PATCH_5E_R4C_R2_VERSION = '5E-R4C-R2-FRONTEND-FLOW-GUARD-CLIENT-METRIC-20260507';
  ...
  function refreshMetricExtra(force, status, extra) {
    return Object.assign({
      ...
      source_layer: 'CLIENT',
      performance_group: force ? 'DIAGNOSTIC' : 'CLIENT_EVENT',
      client_metric_classification: force ? 'DIAGNOSTIC_REFRESH_WORKFLOW' : 'CLIENT_REFRESH_WORKFLOW',
      event_group: force ? 'SUPER_ADMIN_DIAGNOSTIC' : 'SUPER_ADMIN_CLIENT_EVENT',
  ...
  function bindHeader() {
    ...
    if (refresh) refresh.addEventListener('click', function () { refreshAll({ noCache: true, source: 'force_button' }); });
    var refreshCache = byId('sa-refresh-cache');
    if (refreshCache) refreshCache.addEventListener('click', function () { refreshAll({ noCache: false, source: 'cache_button' }); });
  ...
  async function refreshAll(options) {
    ...
    var metricAction = force ? 'superAdminRefreshForce' : 'superAdminRefreshData';

    if (force) {
      var remaining = refreshGuardSecondsLeft();
      if (refreshForceInFlight) {
        updateRefreshButtons('Refresh Paksa sedang berjalan. Mohon tunggu proses saat ini selesai.');
        reportClientPerf('superAdminRefreshForceGuarded', startedAt, refreshMetricExtra(true, 'SUCCESS', {
          skipped: true,
          refresh_guard: 'IN_FLIGHT_CLIENT_GUARD',
          detail: 'Refresh Paksa dilewati karena request sebelumnya masih berjalan.'
        }));
      ...
      if (remaining > 0) {
        ...
        reportClientPerf('superAdminRefreshForceGuarded', startedAt, refreshMetricExtra(true, 'SUCCESS', {
          skipped: true,
          refresh_guard: 'CLIENT_COOLDOWN',
          cooldown_remaining_sec: remaining,
          snapshot_used: !!lastSummary,
          detail: 'Refresh Paksa dilewati di frontend; snapshot terakhir digunakan.'
        }));
    ...
    try {
      if (force) {
        ...
        updateRefreshButtons('Refresh Paksa berjalan. Active tab akan memakai jalur snapshot/cache aman.');
      } else {
      ...
      if (summary) {
        if (force) {
          await reloadActiveTab({ noCache: false, fromForce: true, preferSummary: true });
      ...
      reportClientPerf(metricAction, startedAt, refreshMetricExtra(force, 'SUCCESS', {
        refresh_flow: force ? 'FORCE_FRONTEND_GUARDED' : 'CACHE_REFRESH',
        active_tab_reloaded_with_no_cache: false,
        backend_guard: guardInfo,
        snapshot_or_cache_preferred: force === true,
        detail: force ? 'Refresh Paksa memakai frontend flow guard; active tab tidak dipaksa live-check berat.' : 'Refresh Data memakai cache/snapshot normal.'
      }));
      updateRefreshButtons(force ? 'Refresh Paksa selesai. Guard 60 detik tetap aktif untuk mencegah klik berulang.' : 'Dashboard Super Admin diperbarui.');
      showToast(force ? 'Dashboard diperbarui paksa. Klik berulang akan memakai snapshot selama guard aktif.' : 'Dashboard Super Admin diperbarui.', 'success');
    } catch (err) {
      reportClientPerf(metricAction, startedAt, refreshMetricExtra(force, 'ERROR', {
