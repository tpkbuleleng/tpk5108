(function (window, document) {
  ...
  function workbookCards(data) {
    ...
    return '<div class="sa-grid" style="grid-template-columns:repeat(auto-fit,minmax(210px,1fr));margin-top:10px">' + rows.map(function (w) {
      var key = fmt(w.workbook_key);
      var checked = w.last_checked_at ? formatWita(w.last_checked_at) : 'Belum dicek';
      return [
        '<button type="button" class="sa-card sa-clickable" style="text-align:left" data-sa-workbook-open="', escapeHtml(key), '">',
          ...
          '<small class="sa-card__hint">Status: ', escapeHtml(humanText(w.status || 'NOT_CHECKED')), ' · Isu: ', escapeHtml(fmtNumber(w.issue_count || 0)), '</small>',
          '<small class="sa-card__hint">', escapeHtml(checked), '</small>',
        '</button>'
      ].join('');
    }).join('') + '</div>';
  }
  ...
  function renderWorkbookHealth(data) {
    ...
    var selectedKey = data.selected_workbook_key || selectedWorkbookKey || 'LOG';
    var isDetail = String(data.mode || '').toUpperCase() === 'DETAIL';
    target.innerHTML = [
      '<h3>Workbook & Log Health</h3>',
      '<p class="sa-muted">Mode: ', escapeHtml(humanText(data.mode || 'SUMMARY')), ' · Status: ', badgeStatus(summary.health_status || 'GREEN'), ' · Workbook terdaftar: ', escapeHtml(fmtNumber(summary.total_workbooks || workbooks.length)), ' · Terpilih: ', escapeHtml(selectedKey || '-'), ' · Cache: ', escapeHtml(data.cache_hit ? 'HIT' : 'MISS'), '</p>',
      workbookSummaryCards(data),
      '<section class="sa-panel" style="margin-top:10px"><h3>Daftar Workbook</h3><p class="sa-muted">Klik nama workbook untuk membuka detail sheet/header secara on-demand. Initial load hanya memeriksa LOG agar dashboard tetap cepat.</p>', workbookCards(data), '</section>',
      '<div class="sa-layout" style="grid-template-columns:1fr 1fr;margin-top:10px">',
        '<section class="sa-panel"><h3>Isu Prioritas ', escapeHtml(selectedKey ? '(' + selectedKey + ')' : ''), '</h3>', workbookIssueList(data), '</section>',
      ...
      '</div>',
      '<section class="sa-panel" style="margin-top:10px"><h3>', isDetail ? 'Detail Sheet ' + escapeHtml(selectedKey) : 'Detail LOG Awal', '</h3>',
        '<p class="sa-muted">Baris detail: ', escapeHtml(fmtNumber(rows.length)), '. Untuk workbook selain LOG, klik card workbook di atas.</p>',
        tableHtml(workbookHealthHeaders(), rows, 'workbook'),
      '</section>',
      '<section class="sa-panel" style="margin-top:10px"><h3>Ringkasan Workbook Registry</h3>', tableHtml(workbookListHeaders(), workbooks, 'workbook_summary'), '</section>'
    ].join('');
    bindWorkbookOpenButtons();
    bindDetailRows(rows, 'workbook');
    bindPagination('workbook', function () { renderWorkbookHealth(data); });
    bindDetailRows(workbooks, 'workbook_summary');
  ...
  async function loadWorkbookHealth(options) {
    ...
    var result = await post('getSuperAdminWorkbookHealthSummary', { check_log: true, no_cache: opts.noCache === true });
    if (!result || result.ok === false) return setTabError(result && result.message);
  ...
  async function loadWorkbookDetail(workbookKey, options) {
    ...
    var result = await post('getSuperAdminWorkbookHealthDetail', { workbook_key: selectedWorkbookKey, include_unknown_sheets: opts.includeUnknown === true, write_history: opts.noCache === true, no_cache: opts.noCache === true });
    if (!result || result.ok === false) return setTabError(result && result.message);
    tabLoaded.workbook = true;
    renderWorkbookHealth(getData(result));
  }
