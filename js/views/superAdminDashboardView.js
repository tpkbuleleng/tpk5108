(function (window, document) {
  ...
  var lastSecurityActionTrace = null;
  var lastSecurityActionTracePayload = null;
  ...
  function renderSummary(data) {
    ...
    var diagnosticCriticalItems = data.diagnostic_critical_items || data.diagnostic_items || [];
  ...
  function inferPerformanceGroup(action, category) {
    ...
    var diagnostic = {
      superAdminRefreshForce: true, superAdminWorkbookLiveCheck: true, superAdminWorkbookSnapshotOpen: true,
    ...
    var clientEvent = {
      superAdminRefreshData: true, superAdminWorkbookCardClick: true, superAdminBackendHealthRowClick: true,
      superAdminFrontendHealthRowClick: true, superAdminSecurityActionClick: true, CLIENT_PERFORMANCE: true
    };
    var securityAction = {
      'OPEN SECURITY ACTION CENTER': true, OPEN_MONITOR: true,
      REVOKE_SESSION: true, BLOCK_TARGET: true, UNBLOCK_TARGET: true, SET_COOLDOWN: true, CLEAR_COOLDOWN: true,
      superAdminSecurityAction_revoke: true, superAdminSecurityAction_block: true, superAdminSecurityAction_cooldown: true,
      ...
      revokeSecuritySessions: true, revokeSecuritySession: true, revokeUserSessions: true, revokeTeamSessions: true,
      revokeKecamatanSessions: true, blockSecurityTarget: true, setSecurityCooldown: true,
      unblockSecurityTarget: true, clearSecurityCooldown: true
    };
  ...
  function normalizePolicyRow(row, kind) {
    row = Object.assign({}, row || {});
    var status = String(row.status || '').toUpperCase();
    var expiredAt = row.expired_at || row.expires_at || '';
    if (status === 'ACTIVE' && isExpiredTime(expiredAt)) row.status = 'EXPIRED';
    if (!row.status) row.status = status || '-';
    return row;
  }

  function getSecurityTargetCatalog(data) {
    data = data || {};
    var catalog = data.smart_targets || data.target_catalog || {};
    ...
    var seen = {};
    function push(key, id, label, scope, source) {
      id = String(id || '').trim();
      if (!id) return;
      key = String(key || '').toUpperCase();
      var uniqueKey = key + '|' + id + '|' + String(scope || '');
      if (seen[uniqueKey]) return;
      seen[uniqueKey] = true;
      ensure(key).push({
        target_type: key,
        target_id: id,
        target_scope: String(scope || '').trim(),
        label: label || id,
        source: source || 'ACTIVE_SESSION'
      });
    }
    ...
    sessions.forEach(function (s) {
      ...
      var user = s.id_user || '';
      var role = s.role || '';
      var device = s.device_id || '';
      var deviceLabel = device ? maskDeviceId(device) : '';
      push('USER', user, user + (role ? ' · ' + role : '') + (deviceLabel ? ' · ' + deviceLabel : ''), '', 'ACTIVE_SESSION');
      push('DEVICE', device, deviceLabel + (user ? ' · ' + user : ''), '', 'ACTIVE_SESSION');
      push('USER_DEVICE', user, user + (deviceLabel ? ' · ' + deviceLabel : ''), device, 'ACTIVE_SESSION');
      push('TOKEN', s.token_ref || '', (s.token_ref || '') + (user ? ' · ' + user : ''), '', 'ACTIVE_SESSION');
      push('TEAM', s.id_tim || s.nomor_tim || '', (s.id_tim || s.nomor_tim || '') + (user ? ' · ' + user : ''), '', 'ACTIVE_SESSION');
      push('KECAMATAN', s.id_kecamatan || s.kode_kecamatan || '', (s.id_kecamatan || s.kode_kecamatan || '') + (user ? ' · ' + user : ''), '', 'ACTIVE_SESSION');
    });

    // Alias agar UI yang menampilkan TIM tetap bisa membaca katalog TEAM.
    if (!catalog.TIM && catalog.TEAM) catalog.TIM = catalog.TEAM;
    return catalog;
  }

  function smartTargetPickerHtml(data) {
    var catalog = getSecurityTargetCatalog(data || {});
    var all = [];
    Object.keys(catalog).forEach(function (key) {
      (catalog[key] || []).forEach(function (item) {
        item = item || {};
        var type = String(item.target_type || key || '').toUpperCase();
        all.push({
          type: type,
          id: String(item.target_id || '').trim(),
          scope: String(item.target_scope || '').trim(),
          label: item.label || item.target_id || '',
          source: item.source || ''
        });
      });
    });
    var options = ['<option value="">Pilih target aktif/bermasalah…</option>'];
    all.forEach(function (item) {
      if (!item.id) return;
      options.push([
        '<option value="', escapeHtml(item.id), '"',
        ' data-target-type="', escapeHtml(item.type), '"',
        ' data-target-scope="', escapeHtml(item.scope), '"',
        ' data-source="', escapeHtml(item.source), '">',
        escapeHtml(humanText(item.type) + ' · ' + item.label),
        item.scope ? escapeHtml(' · scope: ' + maskDeviceId(item.scope)) : '',
        item.source ? escapeHtml(' · ' + humanText(item.source)) : '',
        '</option>'
      ].join(''));
    });
    return '<select id="sa-sec-action-smart-target">' + options.join('') + '</select>';
  }

  function bindSmartTargetPicker(data) {
    var typeEl = byId('sa-sec-action-target-type');
    var picker = byId('sa-sec-action-smart-target');
    var targetEl = byId('sa-sec-action-target-id');
    var scopeEl = byId('sa-sec-action-target-scope');
    if (!typeEl || !picker || !targetEl || !scopeEl) return;

    function filterOptions() {
      var selectedType = String(typeEl.value || '').toUpperCase();
      var visibleCount = 0;
      Array.prototype.forEach.call(picker.options, function (opt, idx) {
        if (idx === 0) { opt.hidden = false; return; }
        var optType = String(opt.getAttribute('data-target-type') || '').toUpperCase();
        var visible = !selectedType || optType === selectedType || (selectedType === 'TEAM' && optType === 'TIM') || (selectedType === 'TIM' && optType === 'TEAM');
        opt.hidden = !visible;
        if (visible) visibleCount += 1;
      });
      picker.options[0].textContent = visibleCount ? 'Pilih target ' + humanText(selectedType) + '…' : 'Tidak ada target otomatis untuk ' + humanText(selectedType) + ' — boleh isi manual';
      if (picker.selectedOptions && picker.selectedOptions[0] && picker.selectedOptions[0].hidden) picker.selectedIndex = 0;
    }

    typeEl.addEventListener('change', filterOptions);
    picker.addEventListener('change', function () {
      var opt = picker.selectedOptions && picker.selectedOptions[0];
      if (!opt || !opt.value) return;
      var optType = opt.getAttribute('data-target-type') || typeEl.value || 'USER';
      typeEl.value = optType === 'TIM' ? 'TEAM' : optType;
      targetEl.value = opt.value || '';
      scopeEl.value = opt.getAttribute('data-target-scope') || '';
      filterOptions();
    });
    filterOptions();
  }

  function securityActionFormHtml(data) {
    return [
      '<section class="sa-panel" style="margin-top:12px">',
        ...
        '<div class="sa-filter">',
          ...
          '</select>',
          smartTargetPickerHtml(data || {}),
          '<input id="sa-sec-action-target-id" placeholder="target_id manual: id_user / device_id / token_ref / id_tim / kecamatan">',
  ...
  function securityActionKindToTraceType(kind) {
    kind = String(kind || '').toLowerCase();
    if (kind === 'cooldown') return 'SET_COOLDOWN';
    if (kind === 'block') return 'BLOCK_TARGET';
    if (kind === 'revoke') return 'REVOKE_SESSION';
    if (kind === 'unblock') return 'UNBLOCK_TARGET';
    if (kind === 'clear_cooldown') return 'CLEAR_COOLDOWN';
    return String(kind || '').toUpperCase();
  }

  function buildSecurityActionTracePayload(kind, input, result) {
    input = input || getSecurityActionInput();
    var data = getData(result) || {};
    var out = Object.assign({}, input, {
      action_type: securityActionKindToTraceType(kind),
      target_type: input.target_type || data.target_type || '',
      target_id: input.target_id || data.target_id || '',
      target_scope: input.target_scope || '',
      cooldown_id: data.cooldown_id || input.cooldown_id || '',
      block_id: data.block_id || input.block_id || '',
      policy_id: data.cooldown_id || data.block_id || input.policy_id || '',
  ...
  function securityTraceStageStatus(stage) {
    var s = String(stage && stage.status || '').toUpperCase();
    return badgeStatus(s || 'MISSING');
  }

  function securityTraceEvidenceText(stage) {
    var ev = (stage && stage.evidence && stage.evidence[0]) || null;
    if (!ev) return 'Tidak ada evidence pada sampel.';
    var bits = [];
    if (ev.sheet) bits.push(ev.sheet);
    if (ev.row_number) bits.push('row ' + ev.row_number);
    if (ev.action_type) bits.push(ev.action_type);
    if (ev.target_id) bits.push(ev.target_id);
    if (ev.request_id) bits.push(ev.request_id);
    if (ev.timestamp) bits.push(formatWita(ev.timestamp));
    return bits.join(' · ') || 'Evidence tersedia.';
  }

  function securityActionTracePanelHtml() {
    var trace = lastSecurityActionTrace || null;
    var diagnosis = trace && trace.diagnosis ? trace.diagnosis : null;
    var stages = trace && Array.isArray(trace.stages) ? trace.stages : [];
    var status = diagnosis && diagnosis.status ? diagnosis.status : 'INFO';
    var statusCls = status === 'GREEN' ? 'sa-critical-green' : (status === 'RED' ? 'sa-critical-red' : 'sa-critical-yellow');

    return [
      '<section class="sa-panel" style="margin-top:12px" id="sa-security-action-trace-panel">',
        '<h3>Security Action Trace Verifier</h3>',
        '<p class="sa-muted">Verifier read-only untuk membuktikan kelengkapan transaksi: policy sheet, security_action_log, super_admin_action_log, log_security_event, dan log_performance.</p>',
        '<div class="sa-filter" style="margin-bottom:10px">',
          '<select id="sa-trace-action-type"><option value="SET_COOLDOWN">SET_COOLDOWN</option><option value="BLOCK_TARGET">BLOCK_TARGET</option><option value="REVOKE_SESSION">REVOKE_SESSION</option><option value="UNBLOCK_TARGET">UNBLOCK_TARGET</option><option value="CLEAR_COOLDOWN">CLEAR_COOLDOWN</option></select>',
          '<button type="button" class="sa-btn sa-btn-soft" data-sec-trace="current">Trace Input Saat Ini</button>',
          '<button type="button" class="sa-btn" data-sec-trace="last">Trace Action Terakhir</button>',
        '</div>',
        trace ? [
          '<div class="sa-critical-item ', escapeHtml(statusCls), '">',
            '<strong>', escapeHtml((diagnosis && diagnosis.code) || 'TRACE_RESULT'), '</strong>',
            '<span class="sa-muted">', escapeHtml((diagnosis && diagnosis.message) || ''), '</span>',
            '<div class="sa-footnote">Trace service: ', escapeHtml(trace.service_version || '-'), ' · SecurityActionService aktif: ', escapeHtml(trace.security_action_service_version || '-'), ' · Update: ', escapeHtml(formatWita(trace.generated_at)), '</div>',
          '</div>',
          '<div class="sa-table-wrap" style="margin-top:10px"><table class="sa-table"><thead><tr><th>Tahap</th><th>Status</th><th>Sheet</th><th>Evidence</th></tr></thead><tbody>',
            stages.map(function (stage) {
              return '<tr><td>' + escapeHtml(stage.label || stage.stage || '-') + '</td><td>' + securityTraceStageStatus(stage) + '</td><td>' + escapeHtml(stage.sheet || '-') + '</td><td class="sa-wrap">' + escapeHtml(securityTraceEvidenceText(stage)) + '</td></tr>';
            }).join('') || '<tr><td colspan="4" class="sa-muted">Belum ada stage.</td></tr>',
          '</tbody></table></div>',
          (trace.recommendations && trace.recommendations.length ? '<div class="sa-reco"><strong>Rekomendasi:</strong><ul class="sa-list">' + trace.recommendations.map(function (r) { return '<li>' + escapeHtml(r) + '</li>'; }).join('') + '</ul></div>' : '')
        ].join('') : '<div class="sa-critical-item sa-critical-yellow"><strong>Belum ada trace</strong><span class="sa-muted">Jalankan action atau klik tombol trace untuk membaca bukti dari log.</span></div>',
      '</section>'
    ].join('');
  }

  async function runSecurityActionTrace(payload, options) {
    options = options || {};
    var started = Date.now();
    var tracePayload = Object.assign({ lookback_minutes: 1440, max_rows: 420, no_cache: true }, payload || {});
    lastSecurityActionTracePayload = tracePayload;
    var result = await post('debugSecurityActionTrace', timePayload(tracePayload));
    reportClientPerf('superAdminSecurityActionTrace', started, {
      target_type: tracePayload.target_type || '',
      target_id: tracePayload.target_id || '',
      action_type: tracePayload.action_type || ''
    });
    if (!result || result.ok === false) {
      if (!options.silent) showToast((result && result.message) || 'Trace Security Action gagal.', 'error');
      lastSecurityActionTrace = {
        diagnosis: { code: 'TRACE_ENDPOINT_FAILED', status: 'RED', message: (result && result.message) || 'Endpoint trace gagal.' },
        stages: [],
    ...
    lastSecurityActionTrace = getData(result) || {};
    var code = lastSecurityActionTrace.diagnosis && lastSecurityActionTrace.diagnosis.code;
    var status = lastSecurityActionTrace.diagnosis && lastSecurityActionTrace.diagnosis.status;
    if (!options.silent) showToast('Trace selesai: ' + (code || 'OK'), status === 'RED' ? 'error' : 'success');
    return lastSecurityActionTrace;
  }

  function bindSecurityActionTraceButtons() {
    Array.prototype.forEach.call(document.querySelectorAll('[data-sec-trace]'), function (btn) {
      btn.addEventListener('click', async function () {
        var mode = btn.getAttribute('data-sec-trace') || 'current';
        var payload = null;
        if (mode === 'last' && lastSecurityActionTracePayload) {
          payload = lastSecurityActionTracePayload;
        } else {
          var traceTypeEl = byId('sa-trace-action-type');
          var traceType = traceTypeEl && traceTypeEl.value ? traceTypeEl.value : 'SET_COOLDOWN';
          var pseudoKind = traceType === 'BLOCK_TARGET' ? 'block' : (traceType === 'REVOKE_SESSION' ? 'revoke' : 'cooldown');
          payload = buildSecurityActionTracePayload(pseudoKind, getSecurityActionInput(), null);
          payload.action_type = traceType;
        }
        await runSecurityActionTrace(payload, { silent: false });
  ...
  async function executeSecurityAction(kind, payload) {
    ...
    try {
      var tracePayload = buildSecurityActionTracePayload(kind, payload, result);
      await runSecurityActionTrace(tracePayload, { silent: true });
    } catch (traceErr) {
      lastSecurityActionTrace = {
        diagnosis: { code: 'TRACE_CLIENT_ERROR', status: 'YELLOW', message: String(traceErr && traceErr.message ? traceErr.message : traceErr) },
        stages: [],
        generated_at: new Date().toISOString(),
        service_version: 'client-side'
      };
    }
    await loadSecurityActionCenter({ noCache: true });
  }
  ...
  function renderSecurityActionCenter(data) {
    ...
    target.innerHTML = [
      ...
      '</section>',
      securityActionFormHtml(data),
      securityActionTracePanelHtml(),
    ...
    bindSmartTargetPicker(data);
    bindSecurityActionTraceButtons();
  ...
  async function reloadActiveTab(options) {
    ...
    if (activeTab === 'securityRisk') return loadSecurityRisk({ noCache: opts.noCache === true });
    if (activeTab === 'securityAction') return loadSecurityActionCenter({ noCache: opts.noCache === true });
