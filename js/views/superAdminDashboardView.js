(function (window, document) {
  ...
  async function loadWorkbookHealth(options) {
    ...
    var result = await post('getSuperAdminWorkbookHealth', { scope: opts.scope || 'CRITICAL', include_unknown_sheets: true, no_cache: opts.noCache === true, max_workbooks: opts.maxWorkbooks || 14 });
    if (!result || result.ok === false) return setTabError(result && result.message);
