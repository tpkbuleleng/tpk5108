(function (window, document) {
  ...
  var SYSTEM_MONITOR_FRONTEND_SYNC_VERSION = '5E-R4D-A8-R1-FRONTEND-HEALTH-FALSE-POSITIVE-FILTER-CRITICAL-CLEANUP-20260509';
  ...
  function componentHealthVersionLabel(data, kind) {
    ...
    if (kind === 'frontend') {
      if (String(latest || '').indexOf('5E-R4D-A8-R1') >= 0) return latest;
      return '5E-R4D-A8-R1-FRONTEND-HEALTH-FILTER-ACTIVE';
    }
  ...
  function componentHealthVersionHint(data, kind) {
    ...
    if (kind === 'frontend') {
      ...
      return noise > 0 ? ('Noise eksternal difilter: ' + noise) : 'Frontend health panel dengan false-positive filter';
    }
  ...
  function componentHealthHeaders(kind) {
    if (kind === 'frontend') {
      return [
        ...
        { key: 'signal_source', label: 'Sumber Sinyal' },
        { key: 'noise_filtered_count', label: 'Noise Difilter' },
        { key: 'status', label: 'Status', formatter: function (v) { return badgeStatus(v); } },
        { key: 'recommendation', label: 'Rekomendasi', wrap: true }
      ];
    }
  ...
  function renderComponentHealth(data, kind) {
    ...
    var note = kind === 'frontend'
      ? 'Cek frontend berbasis frontend_asset_registry + sinyal client log yang sudah difilter dari noise browser extension/antivirus. Asset fisik tidak discan dari Drive agar dashboard tetap ringan.'
    ...
    var frontendNoiseInfo = kind === 'frontend'
      ? (' · Error relevan: ' + fmtNumber(summary.relevant_error_count || data.frontend_relevant_error_count || 0) + ' · Noise eksternal difilter: ' + fmtNumber(summary.noise_filtered_count || data.frontend_noise_filtered_count || 0))
      : '';
    var html = [
