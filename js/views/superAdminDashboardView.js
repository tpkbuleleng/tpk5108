(function (window, document) {
  ...
  function isWorkbookCheckedRow(w) {
    if (!w) return false;
    var status = String(w.status || '').toUpperCase();
    return !!(w.last_checked_at || (status && status !== 'NOT_CHECKED' && status !== 'BELUM DICEK') || Number(w.issue_count || 0) > 0 || Number(w.sheet_count || 0) > 0);
  }

  function mergeWorkbookCards(rows) {
    rows = rows || [];

    // Registry cache menyimpan daftar workbook resmi. Hasil cek per workbook disimpan terpisah
    // agar tombol Refresh daftar tidak menghapus status workbook yang sudah dicek di sesi browser.
    ...
    rows.forEach(function (w) {
      ...
      if (isWorkbookCheckedRow(w)) {
        workbookCheckedMap[key] = Object.assign({}, workbookCheckedMap[key] || {}, w);
      }
    });
  ...
  function summaryFromWorkbookCards(baseSummary, cards) {
    var s = Object.assign({}, baseSummary || {});
    cards = cards || [];
    s.total_workbooks = cards.length;
    s.checked_workbooks = 0;
    s.missing_sheet = 0;
    s.header_mismatch = 0;
    s.unknown_sheet = 0;
    s.capacity_warning = 0;
    s.health_status = 'GREEN';
    cards.forEach(function (w) {
      var st = String((w && w.status) || 'NOT_CHECKED').toUpperCase();
      if (st && st !== 'NOT_CHECKED' && st !== 'BELUM DICEK') s.checked_workbooks += 1;
      s.missing_sheet += Number((w && w.missing_sheet) || 0);
      s.header_mismatch += Number((w && w.header_mismatch) || 0);
      s.unknown_sheet += Number((w && w.unknown_sheet) || 0);
      s.capacity_warning += Number((w && w.capacity_warning) || 0);
      if (st !== 'NOT_CHECKED') s.health_status = worseStatusClient(s.health_status, st);
    });
    return s;
  }

  function worseStatusClient(a, b) {
    var rank = { NOT_CHECKED: -1, GREEN: 0, OK: 0, INFO: 0, REVIEW: 1, YELLOW: 1, WARNING: 1, RED: 2, ERROR: 2, CRITICAL: 3 };
    var aa = String(a || 'GREEN').toUpperCase();
    var bb = String(b || 'GREEN').toUpperCase();
  ...
  function bindWorkbookOpenButtons() {
    ...
    Array.prototype.forEach.call(buttons, function (btn) {
      ...
      btn.addEventListener('click', function () {
        ...
        reportClientPerf('superAdminWorkbookCardClick', clickStart, { target_workbook_key: String(key || '').toUpperCase(), workbook_key: String(key || '').toUpperCase(), status: 'SUCCESS' });
  ...
  function renderWorkbookHealth(data) {
    ...
    var summary = summaryFromWorkbookCards(data.summary || {}, cardsForRender);
    ...
    root.innerHTML = [
      ...
      '<section class="sa-panel" style="margin-top:10px"><div class="sa-workbook-tools"><div><h3>Daftar Workbook</h3><p class="sa-muted">Klik nama workbook untuk membuka detail sheet/header secara on-demand. Tampilan awal hanya membaca registry agar dashboard tetap cepat.</p></div><div class="sa-inline-actions">',
        '<button type="button" class="sa-btn sa-btn-soft" id="sa-workbook-refresh-list">Refresh daftar</button>',
      '</div></div>', workbookCards(data), '</section>',
  ...
  async function loadWorkbookHealth(options) {
    ...
    renderWorkbookHealth(getData(result));
  }
  ...
  async function loadWorkbookDetail(workbookKey, options) {
    ...
    setTabLoading('detail workbook ' + selectedWorkbookKey);
    ...
    var result = await post('getSuperAdminWorkbookHealthDetail', {
      ...
      include_optional_registered: opts.includeOptional === true,
      max_sheets: opts.maxSheets || 12,
      write_history: opts.noCache === true,
      no_cache: opts.noCache === true
    });
    if (!result || result.ok === false) return setTabError(result && result.message);
    reportClientPerf('superAdminWorkbookDetailOpen', startedAt, {
  ...
  async function reloadActiveTab(options) {
    ...
    if (activeTab === 'workbook') {
      if (selectedWorkbookKey) return loadWorkbookDetail(selectedWorkbookKey, { noCache: opts.noCache === true });
