(function (window, document) {
  ...
  var SYSTEM_MONITOR_FRONTEND_SYNC_VERSION = '5E-R4D-A8-SYSTEM-MONITOR-FRONTEND-FINAL-SYNC-20260509';
  ...
  function componentHealthVersionLabel(data, kind) {
    ...
    if (kind === 'backend' && String(latest || '').indexOf('5E-R4D-A5') < 0) {
      return 'UI A8 synced · Backend Health final A5 verified';
    }
    if (kind === 'frontend' && !latest) {
      return 'UI A8 synced · Frontend Health active';
    }
  ...
  function componentHealthVersionHint(data, kind) {
    ...
    if (!raw || raw === latest) return kind === 'frontend' ? 'Frontend health panel' : 'Backend health panel';
  ...
  function normalizeCriticalItems(items, limit) {
    ...
    (items || []).forEach(function (item) {
      item = item || {};
      var key = [item.type || '', item.title || '', item.message || ''].join('|').toLowerCase();
  ...
  function renderSummary(data) {
    ...
    var criticalItems = normalizeCriticalItems(critical.items || [], 5);
    var diagnosticCriticalItems = normalizeCriticalItems(data.diagnostic_critical_items || data.diagnostic_items || [], 5);
