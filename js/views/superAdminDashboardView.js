(function (window, document) {
  ...
  function workbookSummaryCards(data) {
    ...
    return [
      '<div class="sa-grid" style="grid-template-columns:repeat(6,minmax(0,1fr));margin-top:0">',
        cardHtml('Workbook dicek', s.total_workbooks || 0, 'Jumlah file', ''),
        cardHtml('Sheet dicek', s.total_rows || 0, 'Termasuk unknown', ''),
        cardHtml('Sheet hilang', s.missing_sheet || 0, 'Missing sheet', s.missing_sheet ? 'sa-danger' : ''),
        cardHtml('Header bermasalah', s.header_mismatch || 0, 'Missing/changed', s.header_mismatch ? 'sa-danger' : ''),
        cardHtml('Sheet tidak dikenal', s.unknown_sheet || 0, 'Perlu review', s.unknown_sheet ? 'sa-warn' : ''),
  ...
  function workbookIssueList(data) {
    ...
    if (!items.length) return '<p class="sa-muted">Tidak ada isu workbook prioritas pada sampel ini.</p>';
  ...
  function renderWorkbookHealth(data) {
    ...
    target.innerHTML = [
      ...
      '<p class="sa-muted">Scope: ', escapeHtml(humanText(data.scope || 'CRITICAL')), ' · Status: ', badgeStatus(summary.health_status || 'GREEN'), ' · Workbook: ', escapeHtml(fmtNumber(summary.total_workbooks || 0)), ' · Baris health: ', escapeHtml(fmtNumber(rows.length)), ' · Cache: ', escapeHtml(data.cache_hit ? 'HIT' : 'MISS'), '</p>',
      workbookSummaryCards(data),
      '<div class="sa-layout" style="grid-template-columns:1fr 1fr;margin-top:10px">',
        '<section class="sa-panel"><h3>Isu Prioritas</h3>', workbookIssueList(data), '</section>',
        '<section class="sa-panel"><h3>Catatan Pemeriksaan</h3><p class="sa-muted">Pemeriksaan ini membaca workbook/sheet/header secara read-only. Sheet <strong>workbook_health_log</strong> menyimpan riwayat isu agar perubahan header/sheet dapat ditelusuri.</p><p class="sa-muted">Gunakan Refresh Paksa bila baru memperbaiki struktur sheet.</p></section>',
      '</div>',
  ...
  async function loadWorkbookHealth(options) {
    ...
    var result = await post('getSuperAdminWorkbookHealth', { scope: opts.scope || 'CRITICAL', include_unknown_sheets: opts.includeUnknown === true, write_history: opts.noCache === true, no_cache: opts.noCache === true, max_workbooks: opts.maxWorkbooks || 2 });
    if (!result || result.ok === false) return setTabError(result && result.message);
  ...
  async function reloadActiveTab(options) {
    ...
    if (activeTab === 'workbook') return loadWorkbookHealth({ noCache: opts.noCache === true });
