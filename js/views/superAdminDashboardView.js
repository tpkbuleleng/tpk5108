(function (window, document) {
  ...
  var SYSTEM_MONITOR_FRONTEND_SYNC_VERSION = '5E-R4D-A8-R2-FRONTEND-ERROR-EVIDENCE-DRILLDOWN-SMARTER-RED-CLASSIFICATION-20260509';
  ...
  function componentHealthVersionLabel(data, kind) {
    ...
    if (kind === 'frontend') {
      if (String(latest || '').indexOf('5E-R4D-A8-R2') >= 0) return latest;
  ...
  function componentHealthVersionHint(data, kind) {
    ...
    if (kind === 'frontend') {
      ...
      if (fatal > 0) return 'Fatal evidence: ' + fatal + ' · Noise difilter: ' + noise;
      if (evidence > 0) return 'Evidence drilldown: ' + evidence + ' · Noise difilter: ' + noise;
  ...
  function detailKeysFor(type, row) {
    ...
    if (type === 'frontend_health') {
      return ['component_key','asset_path','asset_type','load_stage','expected_global','is_required','is_active','status','error_signal','signal_source','classification','evidence_count','fatal_error_count','runtime_error_count','weak_signal_count','direct_match_count','relevant_error_count','noise_filtered_count','last_error_at','last_request_id','last_action','last_module','last_message','evidence_samples','runtime_check','recommendation','catatan'];
    }
  ...
  function renderComponentHealth(data, kind) {
    ...
    var note = kind === 'frontend'
      ? 'Cek frontend berbasis frontend_asset_registry + evidence client log. R2 membedakan fatal/direct runtime, indikasi weak, dan noise eksternal/browser extension agar asset core tidak langsung RED hanya karena disebut di log.'
