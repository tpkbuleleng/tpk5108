(function (window) {
  ...
  var MIRROR_IMPORT_IN_FLIGHT = false;
  var LAST_SYNC_QUEUE_SIGNATURE = '';
  var STORAGE_RUNTIME_GUARD_VERSION = '5E-R4D-A8-R2-R1-STORAGE-SYNC-MIRROR-GUARD-20260510';
  ...
  function buildQueueSignature(queue) {
    try {
      ...
      return safeQueue.map(function (item) {
        item = item || {};
        return [item.queue_id || item.id || '', item.status || item.sync_status || '', item.updated_at || item.created_at || ''].join('|');
      }).join('::').slice(0, 900);
    } catch (err) {
  ...
  async function importLegacyQueue(queue) {
    if (isMirrorGuardActive() || MIRROR_IMPORT_IN_FLIGHT) return [];
    if (!window.QueueRepo || typeof window.QueueRepo.getById !== 'function' || typeof window.QueueRepo.save !== 'function') {
      return [];
    }

    var safeQueue = Array.isArray(queue) ? queue : [];
    var signature = buildQueueSignature(safeQueue);
    if (signature && signature === LAST_SYNC_QUEUE_SIGNATURE) return [];
    LAST_SYNC_QUEUE_SIGNATURE = signature;

    var imported = [];
    MIRROR_IMPORT_IN_FLIGHT = true;
    try {
  ...
  var Storage = {
    ...
    getRuntimeGuardVersion: function () {
      return STORAGE_RUNTIME_GUARD_VERSION;
    }
  };
  ...
  Storage.set = function (key, value) {
    ...
    if (!isMirrorGuardActive() && !MIRROR_IMPORT_IN_FLIGHT && String(key || '').trim() === getSyncQueueKey()) {
      var nextSignature = buildQueueSignature(Array.isArray(value) ? value : []);
      if (nextSignature && nextSignature === LAST_SYNC_QUEUE_SIGNATURE) return result;
