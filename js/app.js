(function (window, document) {
  ...
  var APP_RUNTIME_GUARD_VERSION = '5E-R4D-A8-R2-R1-APP-GLOBAL-ERROR-REPORTER-GUARD-20260510';
  ...
  function shouldSkipRecent(source, action, errorLike) {
    ...
    var now = nowTs();
    var message = getMessage(errorLike).toLowerCase();
    var suppressMs = (message.indexOf('too much recursion') >= 0 || message.indexOf('maximum call stack') >= 0) ? 30000 : 3000;

    if (now - ts < suppressMs) return true;
    recentErrorMap[fp] = now;
    return false;
  }

  function isReporterInternalError(source, action, errorLike, extra) {
    var text = [source || '', action || '', getMessage(errorLike), extra && extra.filename || '', extra && extra.stack_trace || ''].join(' ').toLowerCase();
    if (isReportingError) return true;
    if (window.__TPK_CLIENT_ERROR_REPORTING === true) return true;
    if (text.indexOf('logclienterror') >= 0) return true;
    if (text.indexOf('reportclienterror') >= 0 && text.indexOf('api.js') >= 0) return true;
    if (text.indexOf('__fromclientperformancereporter') >= 0) return true;
    if (text.indexOf('client_performance') >= 0) return true;
    return false;
  }
  ...
  async function reportClientError(source, action, errorLike, extra) {
    ...
    window.__TPK_CLIENT_ERROR_REPORTING = true;
    try {
      await window.Api.reportClientError(message, Object.assign({
        ...
        global_error_reporter_version: APP_RUNTIME_GUARD_VERSION
      }, extra || {}));
      return true;
    } catch (err) {
      return false;
      ...
      window.__TPK_CLIENT_ERROR_REPORTING = false;
    }
  }
  ...
  function installGlobalErrorHandlers() {
    ...
    window.addEventListener('error', function (event) {
      if (window.__TPK_CLIENT_ERROR_REPORTING === true) return;
  ...
  async function startApplication() {
    try {
      log('TPK app starting 3C cold-start diet + A8-R2-R1 runtime guard...');
