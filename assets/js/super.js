// ==========================================
// 👑 GOD MODE: SUPER ADMIN DASHBOARD (V28 - PKB MULTI-VILLAGE SUPPORT)
// ==========================================
...
// [Fungsi Edit Menu & Edit Widget sama seperti V27]
...
export const initSuperAdmin = async (session) => {
    ...
    document.body.innerHTML = `
        ...
        <style>
            ...
            .badge-role { padding: 4px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: bold; text-transform:uppercase; } .role-kader { background: #e8f4fd; color: #0984e3; } .role-admin { background: #fdf3e8; color: #d35400; } .role-super { background: #fbebf0; color: #e94560; } .role-pkb { background: #f3e8ff; color: #6f42c1; } 
...
window.renderSuperView = async (target) => {
    ...
    // --- 🎛️ DASHBOARD UTAMA & ANALITIK ---
    if (target === 'dashboard') { 
        // [HTML DASHBOARD SAMA SEPERTI V27]
        content.innerHTML = `
        ...
        const loadDashboardData = async () => {
            ...
            sheetsAset.forEach(s => { fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'SECURE_GET_ALL', token: SUPER_TOKEN, sheetName: s.name }) }).then(r => r.json()).then(res => { if(res.status === 'success') { let el = document.getElementById(s.id); if(el) el.innerText = res.data.length; } }).catch(e => {}); });
                ...
                if (resU.status === 'success') {
                    ...
                    window.dashAnalyticData.users = rawUsers.map(u => {
                        ...
                        if (role.includes('PKB')) {
                            // Untuk PKB biarkan nDesa tetap u.scope_desa agar menampung banyak desa
                            if(nDesa === '-' || nDesa === '') {
    ...
    // --- 🛡️ MENU AUDIT LOG ---
        ...
        // [HTML KODE AUDIT TRAIL SAMA SEPERTI V27]
        content.innerHTML = `
    ...
    // --- 👥 MENU MANAJEMEN PENGGUNA ---
        ...
        content.innerHTML = `
            ...
            <div class="super-card" style="padding:0; overflow:hidden;">
                <div style="background:#f8f9fa; padding:15px; border-bottom:1px solid #eee; display:flex; gap:10px; flex-wrap:wrap; align-items:center;">
                    ...
                    <select id="flt-role" class="filter-input"><option value="ALL">📋 Semua Role</option><option value="KADER">KADER</option><option value="PKB">PKB / Penyuluh</option><option value="ADMIN_KECAMATAN">ADMIN KECAMATAN</option><option value="ADMIN_KABUPATEN">ADMIN KABUPATEN</option></select>
            ...
            <div id="modal-add-user" style="display:none; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.7); z-index:1000; align-items:center; justify-content:center;">
                <div style="background:white; padding:30px; border-radius:10px; width:90%; max-width:550px; max-height:90vh; overflow-y:auto; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                    ...
                    <div style="margin-bottom:15px;"><label style="display:block; font-size:0.85rem; font-weight:bold; margin-bottom:5px;">ID Pengguna / Kunci Masuk</label><input type="text" id="add-id" class="filter-input" placeholder="Cth: TPK9999 / PKB001" style="width:100%; box-sizing:border-box;" required></div>
                    ...
                    <div id="panel-kader-area" style="display:block; background:#eef2f5; padding:15px; border-radius:8px; margin-bottom:15px; border:1px solid #dcdde1;">
                        <p id="lbl-penempatan" style="margin:0 0 10px 0; font-size:0.85rem; font-weight:bold; color:#0984e3;">📍 Penempatan Tugas Kader</p>
        ...
        try {
            ...
            if (res.status === 'success') {
                ...
                // 🔥 V28: Populate untuk semua dropdown Desa
                const populateDesaDropdown = () => {
                    const selectedKec = formKecSelect.value; 
                    const defOpt = '<option value="">-- Pilih Desa / Kelurahan --</option>';
                    formDesaSelect.innerHTML = defOpt;
                    document.getElementById('add-desa-2').innerHTML = defOpt;
                    document.getElementById('add-desa-3').innerHTML = defOpt;
                    document.getElementById('add-desa-4').innerHTML = defOpt;
                    document.getElementById('add-desa-5').innerHTML = defOpt;
                    ...
                    Array.from(desaSet).sort().forEach(d => { 
                        ...
                        formDesaSelect.innerHTML += opt;
                        document.getElementById('add-desa-2').innerHTML += opt;
                        document.getElementById('add-desa-3').innerHTML += opt;
                        document.getElementById('add-desa-4').innerHTML += opt;
                        document.getElementById('add-desa-5').innerHTML += opt;
                    });
                };
                ...
                roleSelect.addEventListener('change', () => { 
                    ...
                    if (roleSelect.value === 'ADMIN_KABUPATEN') { 
                        ...
                        formKecSelect.style.backgroundColor = 'white'; formKecSelect.style.pointerEvents = 'auto'; if(formKecSelect.value === 'ALL') formKecSelect.value = 'GEROKGAK'; panelKader.style.display = 'none'; 
                        ...
                        boxPkb.style.display = 'block'; // Tampilkan slot Desa 2-5
                        document.getElementById('lbl-penempatan').innerHTML = "📍 Wilayah Binaan PKB"; 
                        lblDesa1.innerHTML = "1. Pilih Desa / Kelurahan Binaan 1 <span style='color:red; font-weight:bold;'>(Wajib)</span>";
                        populateDesaDropdown(); 
                    } else { 
                        formKecSelect.style.backgroundColor = 'white'; formKecSelect.style.pointerEvents = 'auto'; if(formKecSelect.value === 'ALL') formKecSelect.value = 'GEROKGAK'; 
                        panelKader.style.display = 'block'; document.getElementById('box-add-tim').style.display = 'block'; 
                        boxPkb.style.display = 'none'; // Sembunyikan slot PKB
                        document.getElementById('lbl-penempatan').innerHTML = "📍 Penempatan Tugas Kader"; 
                        lblDesa1.innerHTML = "1. Pilih Desa / Kelurahan";
                        populateDesaDropdown(); 
                    } 
                }); 
                ...
                document.getElementById('btn-submit-add').onclick = async () => {
                    ...
                    // 🔥 V28: Cek Validasi Berdasarkan Role
                    if (role === 'KADER') { 
                        ...
                        const d1 = formDesaSelect.value;
                        const d2 = document.getElementById('add-desa-2').value;
                        const d3 = document.getElementById('add-desa-3').value;
                        const d4 = document.getElementById('add-desa-4').value;
                        const d5 = document.getElementById('add-desa-5').value;

                        if(!d1 || !d2) { alert("⚠️ Mohon lengkapi minimal Desa Binaan 1 dan 2 untuk PKB!"); return; } 
                        
                        // Gabungkan desa, buang duplikat otomatis menggunakan Set
                        const arrDesa = [d1, d2, d3, d4, d5].filter(Boolean);
                        finalDesa = [...new Set(arrDesa)].join(', ');
                    }
    ...
    // [Menu lainnya (Kuesioner, Widget, dll) SAMA dengan V27]
};
