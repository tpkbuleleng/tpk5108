// ==========================================
// 👑 GOD MODE: SUPER ADMIN DASHBOARD (V45 - DYNAMIC TOKEN PATCH)
// ==========================================
...
window.getSuperToken = () => {
    if (window.sessionSuperToken) return window.sessionSuperToken;
    const token = prompt("🛡️ OTENTIKASI SUPER ADMIN\n\nSistem mendeteksi akses ke jalur rahasia (Secure API).\nMasukkan Token Super Admin Anda:");
    if (token) {
        window.sessionSuperToken = token;
        return token;
    }
    return "TOKEN_KOSONG";
};

window.secureFetch = async (payload) => {
    const token = window.getSuperToken();
    if (token === "TOKEN_KOSONG") return { status: 'error', message: 'Dibatalkan oleh Pengguna.' };
    payload.token = token;
    const response = await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify(payload) });
    const res = await response.json();
    if (res.status === 'error' && res.message && res.message.includes('Token')) {
        window.sessionSuperToken = null; 
        alert("❌ Akses Ditolak! Token salah. Silakan coba klik lagi.");
    }
    return res;
};
...
window.renderUserTable = () => {
    try {
        ...
        const kecEl = document.getElementById('flt-kec'); 
    ...
    try {
        if (target === 'dashboard') { 
            ...
            const loadDashboardData = async () => {
                window.getSuperToken(); // Pancing prompt sekali di awal jika token belum ada di RAM
                ...
                sheetsAset.forEach(s => { window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: s.name }).then(res => { if(res.status === 'success') { let el = document.getElementById(s.id); if(el) el.innerText = res.data.length; } }).catch(e => { catatErrorSistem(`Load Dashboard - ${s.name}`, e); }); });

                try {
                    const [resU, resT, resK, resA, resP] = await Promise.all([
                        window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'USER_LOGIN' }),
                        window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'MASTER_TIM' }),
                        window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'MASTER_KADER' }),
                        window.secureFetch({ action: 'SECURE_GET_AUDIT' }),
                        window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'MASTER_PKB' })
                    ]);
            ...
            try {
                const res = await window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'MASTER_PENGUMUMAN' });
                if(res.status === 'success') {
                    ...
                    document.getElementById('btn-submit-p').onclick = async () => {
                        ...
                        try {
                            const res = await window.secureFetch({ action: 'SECURE_ADD_PENGUMUMAN', data: payloadData });
            ...
            try {
                ...
                const res = await window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'USER_LOGIN' });
                ...
                if (res.status === 'success') {
                    ...
                    document.getElementById('btn-submit-add').onclick = async () => {
                        ...
                        try { 
                            const res = await window.secureFetch({ action: 'SECURE_ADD_USER', data: payloadData, data_kader: payloadKader }); 
                            if(res.status === 'success') { alert("✅ Pengguna berhasil ditambahkan!"); document.getElementById('modal-add-user').style.display = 'none'; document.getElementById('table-wrapper').innerHTML = `<div style="padding:50px; text-align:center; color:#0043A8;"><h3>🔄 MENYINKRONKAN DATABASE...</h3><p>Memuat data terbaru dari Google Server.</p></div>`; await clearStore('master_kader'); await clearStore('master_tim'); setTimeout(() => { window.renderSuperView('user_management'); }, 1000); } else { alert("❌ Gagal menyimpan: " + res.message); } 
            ...
            `;
            try {
                const [resW, resM] = await Promise.all([ window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'MASTER_WIDGET' }), window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'MASTER_MENU' }) ]);
                if (resW.status === 'success') { 
                    ...
                    if(btnSubmitW) btnSubmitW.onclick = async () => {
                        ...
                        try { 
                            const res = await window.secureFetch({ action: actionType, data: payloadW }); 
            ...
            try {
                if(window.superTimData.length === 0) {
                    const res = await window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'MASTER_TIM' });
            ...
            try {
                if(window.superUsersData.length === 0) {
                    const resU = await window.secureFetch({ action: 'SECURE_GET_ALL', sheetName: 'USER_LOGIN' });
                    if(resU.status === 'success') window.superUsersData = resU.data || [];
                }

                const res = await window.secureFetch({ action: 'SECURE_GET_AUDIT' });
