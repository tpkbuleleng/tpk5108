// ==========================================
// 👑 GOD MODE: SUPER ADMIN DASHBOARD (V39 - RADAR ANOMALI ACTIVE)
// ==========================================
...
window.activeAuditTab = 'CCTV'; // Toggle for Audit Log
    ...
    try {
        ...
        window.superUsersData.forEach((u) => {
            ...
            if (matchSearch && matchRole && matchKec) {
                ...
                else if (role.includes('KABUPATEN')) badgeClass = 'role-adminkab';
                else if (role.includes('KECAMATAN') || role === 'ADMIN') badgeClass = 'role-adminkec';
                else if (role === 'ADMIN_DESA') badgeClass = 'role-desa';
                else if (role.includes('PKB')) badgeClass = 'role-pkb';
                else if (role.includes('MITRA')) badgeClass = 'role-mitra';
...
window.renderAuditTable = () => { 
    try { 
        // 🔥 V39: Render Log ERROR_LOG jika tab Error yang aktif
        if (window.activeAuditTab === 'ERROR') {
            ...
            const sortedErr = [...window.superErrorData].sort((a,b) => new Date(b.waktu) - new Date(a.waktu));
            
            sortedErr.forEach(err => {
                const dt = new Date(err.waktu);
                if (isNaN(dt.getTime())) return;
                const usr = String(err.id_pengguna).toLowerCase();
                const loc = String(err.lokasi_sistem).toLowerCase();
                const msg = String(err.pesan_error).toLowerCase();
                if (searchVal && !usr.includes(searchVal) && !loc.includes(searchVal) && !msg.includes(searchVal)) return;

                count++;
                tableHtml += `<tr>
                    <td style="color:#666; font-size:0.85rem;">${dt.toLocaleString('id-ID')}</td>
                    <td><b style="color:#e94560;">${err.id_pengguna || '-'}</b></td>
                    <td><b style="color:#0A2342; font-size:0.85rem;">${err.lokasi_sistem || '-'}</b></td>
                    <td><div style="font-size:0.7rem; color:#666; max-height:45px; overflow:hidden;" title="${err.perangkat}">${err.perangkat || '-'}</div></td>
                    <td><code style="font-size:0.8rem; color:#d63031; background:#fbebf0; padding:4px 6px; border-radius:4px; display:block; white-space:pre-wrap; word-break:break-all;">${err.pesan_error || '-'}</code></td>
                </tr>`;
            });
            if (count === 0) tableHtml += `<tr><td colspan="5" style="text-align:center; padding:40px; color:#198754; font-size:1.1rem; font-weight:bold;">✅ Mesin Bersih! Tidak ada Error yang terekam.</td></tr>`;
...
window.renderSuperView = async (target) => {
    ...
    try {
                ...
                try {
                    ...
                    if (resU.status === 'success') {
                        ...
                        if(resA.status === 'success') {
                            ...
                            window.superErrorData = resA.error_data || []; // 🔥 V39: Tangkap Data Error
                            ...
                            // 🔥 V39: Render Daftar Error ke Dashboard
        ...
        // --- 🛡️ MENU AUDIT LOG ---
            ...
            window.activeAuditTab = 'CCTV'; // Set tab awal ke CCTV
            ...
            try {
                ...
                if(resA.status === 'success' && resU.status === 'success') {
                    ...
                    window.superErrorData = resA.error_data || []; // 🔥 V39: Simpan data Error
                    ...
                    // 🔥 V39: Event Listener untuk TAB Toggle
                    document.getElementById('btn-tab-cctv').onclick = () => {
                        ...
                        document.getElementById('log-flt-aksi').style.display = 'block'; // Tampilkan filter aksi
                        window.renderAuditTable();
                    };
                    document.getElementById('btn-tab-error').onclick = () => {
                        ...
                        document.getElementById('log-flt-aksi').style.display = 'none'; // Sembunyikan filter aksi (tidak relevan untuk error)
                        window.renderAuditTable();
                    };
            ...
            try {
                ...
                if (res.status === 'success') {
                    window.superUsersData = res.data.map(u => {
                        ...
                        // BUG FIX: Gunakan kaderData (bukan kaders)
                    ...
                    const selectKec = document.getElementById('flt-kec-filter'); Array.from(kecSet).sort().forEach(k => { const opt = document.createElement('option'); opt.value = k; opt.innerText = k; selectKec.appendChild(opt); });
                    const searchInput = document.getElementById('flt-search'); const clearBtn = document.getElementById('btn-clear-search');
                    window.renderUserTable(); searchInput.addEventListener('input', () => { clearBtn.style.display = searchInput.value ? 'block' : 'none'; window.renderUserTable(); }); clearBtn.addEventListener('click', () => { searchInput.value = ''; clearBtn.style.display = 'none'; searchInput.focus(); window.renderUserTable(); }); document.getElementById('flt-role').addEventListener('change', window.renderUserTable); document.getElementById('flt-kec-filter').addEventListener('change', window.renderUserTable);
