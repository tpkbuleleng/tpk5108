// ==========================================
// 👑 GOD MODE: SUPER ADMIN DASHBOARD (V36 - STRICT BLUE & GOLD PALETTE FIXED)
// ==========================================
...
// ==========================================
// 2. FUNGSI RENDER TABEL (Didefinisikan HANYA SEKALI di sini)
// ==========================================
    ...
    try {
        ...
        window.superUsersData.forEach((u) => {
            ...
            if (matchSearch && matchRole && matchKec) {
                ...
                // 🔥 V36: Logika Warna Ketat (Strict Palette)
                ...
                else if (role.includes('DESA')) badgeClass = 'role-desa';
    ...
    try {
        ...
        document.body.innerHTML = `
            ...
            <style>
                ...
                /* 🔥 V36: STRICT BLUE & GOLD LENCANA PALETTE */
                ...
                .role-kader { background: #E8F4FD; color: #2980B9; border-color: #b6d4fe; } /* Light Blue + Medium Blue */
                .role-adminkab { background: #0043A8; color: #F1C40F; border-color: #0043A8; } /* Royal Blue + Gold */
                .role-adminkec { background: #2980B9; color: #FFFFFF; border-color: #2980B9; } /* Steel Blue + White */
                .role-desa { background: #F1C40F; color: #0A2342; border-color: #F1C40F; } /* Bright Gold + Dark Blue */
                .role-pkb { background: #B8860B; color: #FFFFFF; border-color: #B8860B; } /* Dark Gold/Mustard + White */
                .role-mitra { background: #FFF8E7; color: #2980B9; border-color: #D4AF37; } /* Pale Gold + Medium Blue */
                .role-super { background: #0A2342; color: #F1C40F; border-color: #F1C40F; } /* Navy Blue + Bright Gold */
        ...
        if (target === 'dashboard') { 
                    ...
                    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 15px;">
                        ...
                        <div class="super-card">
                            ...
                            <div style="display:grid; grid-template-columns: 1fr; gap:10px; margin-top:15px;">
                                <button class="quick-link-btn" onclick="document.querySelector('[data-target=user_management]').click()" style="padding:15px; background:#f8f9fa; border:1px solid #ddd; border-radius:8px; cursor:pointer; text-align:left; font-weight:bold; color:#0A2342; transition:all 0.2s;">👥 Kelola Pengguna</button>
                                <button class="quick-link-btn" onclick="document.querySelector('[data-target=kuesioner]').click()" style="padding:15px; background:#f8f9fa; border:1px solid #ddd; border-radius:8px; cursor:pointer; text-align:left; font-weight:bold; color:#0A2342; transition:all 0.2s;">📋 Rakit Kuesioner</button>
                                <button class="quick-link-btn" onclick="document.querySelector('[data-target=widget_management]').click()" style="padding:15px; background:#f8f9fa; border:1px solid #ddd; border-radius:8px; cursor:pointer; text-align:left; font-weight:bold; color:#0A2342; transition:all 0.2s;">🧩 Buat Widget</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            content.innerHTML = `
                <div class="super-card" style="margin-bottom:20px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px; border-left:4px solid #F1C40F;">
                    <div><h3 style="margin:0; color:#0A2342;">🛡️ Audit Log & Rincian CCTV</h3><p style="margin:5px 0 0 0; color:#666; font-size:0.9rem;">Rincian aktivitas pengguna dan log keamanan sistem.</p></div>
                </div>
                ...
                <div class="super-card" style="padding:0; overflow:hidden;">
                    ...
                    <div id="table-wrapper-log" class="super-table-container"><div style="padding:50px; text-align:center; color:#0043A8;"><h3>⏳ Menyedot Rekaman CCTV...</h3></div></div>
                </div>
            `;
                ...
                if (res.status === 'success') {
                    ...
                    document.getElementById('btn-submit-add').onclick = async () => {
                        ...
                        try {
                            ...
                            if(res.status === 'success') { alert("✅ Pengguna berhasil ditambahkan!"); modalAdd.style.display = 'none'; await clearStore('master_kader'); await clearStore('master_tim'); document.getElementById('table-wrapper').innerHTML = `<div style="padding:50px; text-align:center; color:#0043A8;"><h3>🔄 MENYINKRONKAN DATABASE...</h3><p>Memuat data terbaru dari Google Server.</p></div>`; setTimeout(() => { window.renderSuperView('user_management'); }, 1000); } else { alert("❌ Gagal menyimpan: " + res.message); }
