// ==========================================
// 👑 GOD MODE: SUPER ADMIN DASHBOARD (V43 - ANTI NULL & SAFE RENDER)
// ==========================================
...
window.renderUserTable = () => {
    try {
        ...
        if (!searchEl || !roleEl || !kecEl) return; // 🔥 SAFE RENDER: Cegah error Null
...
window.renderKuesionerTable = () => { 
    try { 
        ...
        if (!fltKatEl) return; // 🔥 SAFE RENDER
...
window.renderAuditTable = () => { 
    try { 
        if (window.activeAuditTab === 'ERROR') {
            ...
            if (!searchEl) return; // 🔥 SAFE RENDER
        ...
        const searchEl = document.getElementById('log-flt-search');
        if (!waktuEl || !aksiEl || !searchEl) return; // 🔥 SAFE RENDER
    ...
    try {
        if (target === 'dashboard') { 
            ...
            const renderAnalytics = () => {
                ...
                if (!wktEl || !roleEl || !kecEl || !desaEl) return; // 🔥 SAFE RENDER
        ...
        // --- 🛡️ MENU AUDIT LOG ---
            ...
            try {
                ...
                if(resA.status === 'success' && resU.status === 'success') {
                    ...
                    const wktEl = document.getElementById('log-flt-waktu');
                    const aksiEl = document.getElementById('log-flt-aksi');
                    const searchEl = document.getElementById('log-flt-search');
                    
                    if(wktEl) wktEl.addEventListener('change', window.renderAuditTable); 
                    if(aksiEl) aksiEl.addEventListener('change', window.renderAuditTable); 
                    if(searchEl) searchEl.addEventListener('input', window.renderAuditTable);
                    ...
                    document.getElementById('btn-tab-cctv').onclick = () => {
                        ...
                        const fltAksi = document.getElementById('log-flt-aksi');
                        if (fltAksi) fltAksi.style.display = 'block'; 
                        window.renderAuditTable();
                    };
                    document.getElementById('btn-tab-error').onclick = () => {
                        ...
                        const fltAksi = document.getElementById('log-flt-aksi');
                        if (fltAksi) fltAksi.style.display = 'none'; 
                        window.renderAuditTable();
                    };
