...
const renderMenu = (role) => {
    ...
    const menus = [
        ...
        { id: 'kalkulator', icon: '🧮', label: 'Kalkulator Cerdas' },
    ...
    if (filteredQ.length > 0) {
        ...
        filteredQ.forEach(q => {
            ...
            if(q.tipe_input === 'select') {
                ...
                inputHtml = `<select name="${q.id_pertanyaan}" id="${q.id_pertanyaan}" class="form-control" ${req}><option value="">-- Pilih Jawaban --</option>${opsiHtml}</select>`;
            } else {
                let pHolder = q.tipe_input === 'number' ? 'Masukkan angka...' : 'Ketik jawaban...';
                inputHtml = `<input type="${q.tipe_input || 'text'}" name="${q.id_pertanyaan}" id="${q.id_pertanyaan}" class="form-control" placeholder="${pHolder}" step="any" ${req}>`;
            }
        ...
        if (riwayat.length === 0) {
            ...
            htmlRiwayat = riwayat.map(p => {
                ...
                for (const [key, value] of Object.entries(p.data_laporan || {})) {
                    ...
                    else { label = key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()); }
