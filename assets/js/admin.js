...
const fetchAdminData = async () => {
    try {
        ...
        const [response, masterRes] = await Promise.all([ fetch(url), fetch(SCRIPT_URL) ]);
...
const exportTableToExcel = (tableID, filename = '') => {
    ...
    const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = filename ? filename + '.xls' : 'Rekapan_Laporan.xls'; document.body.appendChild(link); link.click(); document.body.removeChild(link);
};
...
window.renderAdminView = async (target) => {
    ...
    if (target === 'dashboard') {
            ...
            <div class="animate-fade">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 15px;">
                    ...
                    <div style="background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); border: 1px solid #e1e8ed;">
                        ...
                        ${topKader.map((k, idx) => {
                            return `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; border-bottom:1px dashed #eee; padding-bottom:8px;"><div style="font-size:0.85rem;"><span style="color:var(--th-accent); font-weight:bold; margin-right:5px;">#${idx+1}</span> <b style="color:var(--th-dark);">${window.getKaderName(k[0])}</b></div><div style="font-size:0.8rem; background:var(--th-light); color:var(--th-main); padding:2px 8px; border-radius:10px; font-weight:bold; border:1px solid var(--th-main);">${k[1]} Lapor</div></div>`
                        }).join('')}
                        </div>
                    </div>
                </div>
            ...
            <div class="animate-fade">
                ...
                <div style="background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); margin-bottom: 20px; border: 1px solid #e1e8ed;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 15px;">
                        ...
                        <select id="flt-tpk-unsur" class="admin-input"><option value="ALL">🎓 Semua Unsur Kader</option><option value="BIDAN">Bidan / Tenaga Kesehatan</option><option value="PKK">Kader PKK</option><option value="KB">Kader KB</option></select>
                        <select id="flt-tpk-bpjs" class="admin-input"><option value="ALL">🏥 Status BPJSTK (Semua)</option><option value="YA">Memiliki BPJSTK</option><option value="TIDAK">Belum Memiliki</option></select>
                    </div>
                </div>
        ...
        if (roleUpper === 'ADMIN_DESA' || roleUpper.includes('PKB')) {
            const allowedDesa = String(window.adminSession.desa || '').toUpperCase().split(',').map(d => d.trim());
            if (!allowedDesa.includes('ALL') && !allowedDesa.includes('-') && allowedDesa.length > 0 && allowedDesa[0] !== "") { dataKaderLokal = masterKader.filter(k => allowedDesa.includes(String(k.desa_kelurahan || k.desa).toUpperCase())); } 
            else if (roleUpper.includes('PKB')) { dataKaderLokal = masterKader.filter(k => String(k.kecamatan).toUpperCase() === String(window.adminSession.kecamatan).toUpperCase()); }
        }
        ...
        if (elKec) {
            ...
            if (!roleUpper.includes('KABUPATEN') && !roleUpper.includes('SUPER') && !roleUpper.includes('MITRA')) { elKec.value = window.adminSession.kecamatan; elKec.disabled = true; }
        }
        if (elDesa) {
            ...
            if (roleUpper === 'ADMIN_DESA') { elDesa.value = window.adminSession.desa; elDesa.disabled = true; }
        }
        ...
        const renderTableKader = () => {
            ...
            let finalData = dataKaderLokal.filter(k => {
                ...
                let unsurKader = String(k.unsur_tpk || k.unsur || k.unsur_kader || '').toUpperCase(); let matchUnsur = valUnsur === 'ALL' || unsurKader.includes(valUnsur);
            ...
            finalData.forEach(k => {
                ...
                if (u.includes('BIDAN') || u.includes('NAKES')) statNakes++; else if (u.includes('PKK')) statPkk++; else if (u.includes('KB')) statKb++;
                if (String(k.bpjs || k.status_bpjs).toUpperCase() === 'YA') statBpjs++; if (String(k.mbg || k.mengantar_mbg).toUpperCase() === 'YA') statMbg++;
            });

            document.getElementById('tpk-stats-container').innerHTML = `
                <div style="background:white; border-bottom:4px solid var(--th-main); padding:20px; border-radius:8px; box-shadow:0 2px 5px rgba(0,0,0,0.05); text-align:center;"><div style="font-size:2rem; font-weight:900; color:var(--th-main);">${finalData.length}</div><div style="font-size:0.8rem; font-weight:bold; color:#666;">TOTAL KADER</div></div>
                <div style="background:white; border-bottom:4px solid #198754; padding:20px; border-radius:8px; box-shadow:0 2px 5px rgba(0,0,0,0.05); text-align:center;"><div style="font-size:1.4rem; font-weight:900; color:#198754;">${statNakes} <span style="font-size:0.9rem; color:#666;">Bidan</span></div><div style="font-size:0.8rem; color:#666; font-weight:bold; margin-top:5px;">${statPkk} PKK | ${statKb} KB</div></div>
                <div style="background:white; border-bottom:4px solid #fd7e14; padding:20px; border-radius:8px; box-shadow:0 2px 5px rgba(0,0,0,0.05); text-align:center;"><div style="font-size:2rem; font-weight:900; color:#fd7e14;">${statBpjs}</div><div style="font-size:0.8rem; font-weight:bold; color:#666;">PUNYA BPJSTK</div></div>
                <div style="background:white; border-bottom:4px solid #6f42c1; padding:20px; border-radius:8px; box-shadow:0 2px 5px rgba(0,0,0,0.05); text-align:center;"><div style="font-size:2rem; font-weight:900; color:#6f42c1;">${statMbg}</div><div style="font-size:0.8rem; font-weight:bold; color:#666;">PENYALUR MBG</div></div>
            `;
            ...
            if (finalData.length === 0) { tbody.innerHTML = '<tr><td colspan="6" style="padding: 30px; text-align: center; color: #dc3545; font-weight:bold; font-size:1.1rem;">❌ Tidak ada data kader yang cocok dengan filter.</td></tr>'; } 
                ...
                tbody.innerHTML = finalData.map((k, idx) => {
                    let stBpjs = String(k.bpjs || k.status_bpjs).toUpperCase() === 'YA' ? '<span style="background:#e8f4fd; color:var(--th-main); padding:4px 10px; border-radius:12px; font-size:0.75rem; font-weight:bold; border:1px solid #b6d4fe;">✅ Terdaftar</span>' : '<span style="background:#fdf3e8; color:#d35400; padding:4px 10px; border-radius:12px; font-size:0.75rem; font-weight:bold; border:1px solid #fadbd8;">❌ Belum</span>';
                    let stMbg = String(k.mbg || k.mengantar_mbg).toUpperCase() === 'YA' ? '<span style="background:#e2f0cb; color:#198754; padding:4px 10px; border-radius:12px; font-size:0.75rem; font-weight:bold; border:1px solid #c3e6cb;">🍲 Penyalur</span>' : '<span style="color:#aaa; font-size:0.8rem; font-weight:bold;">-</span>';
                    let un = String(k.unsur_tpk || k.unsur || k.unsur_kader || '').toUpperCase(); let badgeUnsur = (un.includes('BIDAN') || un.includes('NAKES')) ? '🏥 Nakes/Bidan' : (un.includes('PKK') ? '👩‍🌾 Kader PKK' : '🔵 Kader KB');
                    let jk = String(k.jenis_kelamin || '').toUpperCase(); let charJk = (jk === 'LK' || jk === 'LAKI-LAKI') ? 'L' : ((jk === 'PR' || jk === 'PEREMPUAN') ? 'P' : '-');
                    let myDusun = timWilayah.filter(tw => String(tw.id_tim) === String(k.id_tim)).map(tw => tw.dusun_rw).join(', '); if (!myDusun || myDusun.trim() === '') myDusun = '-';
        ...
        content.innerHTML = `
            <div class="animate-fade">
                ...
                <div style="background:white; padding:15px; border-radius:8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); border: 1px solid #e1e8ed;">
                    <div style="display:flex; gap:10px; margin-bottom:15px; flex-wrap: wrap;"><input type="text" id="flt-sasaran-nama" class="admin-input" placeholder="Cari Nama/NIK/Desa..." style="flex:2; min-width: 200px;"><select id="flt-sasaran-jenis" class="admin-input" style="flex:1; min-width: 150px;"><option value="ALL">Semua Jenis</option><option value="CATIN">CATIN</option><option value="BUMIL">BUMIL</option><option value="BUFAS">BUFAS</option><option value="BADUTA">BADUTA</option></select></div>
                    <div style="overflow-x:auto;"><table class="admin-table"><thead><tr><th>ID Registrasi</th><th>Tgl Daftar</th><th>Jenis</th><th>Nama Sasaran</th><th>Desa / Dusun</th><th>Kader Pendata</th><th>Status</th></tr></thead><tbody id="tbody-sasaran"></tbody></table></div>
                </div>
            </div>
        `;
        ...
        const renderTabelSasaran = () => {
            ...
            tbody.innerHTML = filtered.map(r => {
                const tgl = new Date(r.created_at).toLocaleDateString('id-ID'); const namaKaderAsli = window.getKaderName(r.username);
                ...
                return `<tr style="border-bottom:1px solid #eee;"><td><code style="color:var(--th-dark); font-weight:bold;">${r.id}</code></td><td>${tgl}</td><td><b style="color:#B8860B;">${r.jenis_sasaran}</b></td><td style="color:var(--th-main); font-weight:bold;">${r.nama_sasaran}</td><td>${r.desa}<br><span style="font-size:0.75rem; color:#888;">${r.dusun}</span></td><td>${namaKaderAsli}</td><td>${statBadge}</td></tr>`;
            }).join('');
        };
        ...
        const btnKec = document.getElementById('dash-flt-kec'); if (btnKec) btnKec.addEventListener('change', () => { window.currentFilterKec = btnKec.value; window.currentFilterDesa = 'ALL'; window.renderAdminView('sasaran'); });
        const btnDesa = document.getElementById('dash-flt-desa'); if (btnDesa) btnDesa.addEventListener('change', () => { window.currentFilterDesa = btnDesa.value; window.renderAdminView('sasaran'); });
    }
        ...
        content.innerHTML = `
            <div class="animate-fade">
                ...
                ${filterWilayahHTML}
                <div style="background:white; padding:15px; border-radius:8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); border: 1px solid #e1e8ed;">
                    <div style="display:flex; gap:10px; margin-bottom:15px; flex-wrap: wrap;"><input type="text" id="flt-pend-nama" class="admin-input" placeholder="Cari ID Sasaran / Petugas..." style="flex:2; min-width: 200px;"></div>
                    <div style="overflow-x:auto;"><table class="admin-table"><thead><tr><th>Tgl Kunjungan</th><th>ID Sasaran (Target)</th><th>Kader Pelapor</th><th>Catatan Lapangan</th><th>Lokasi GPS</th></tr></thead><tbody id="tbody-pend"></tbody></table></div>
                </div>
            </div>
        `;
        ...
        const renderTabelPend = () => {
            ...
            tbody.innerHTML = filtered.map(p => {
                ...
                const tgl = rData.tgl_kunjungan ? new Date(rData.tgl_kunjungan).toLocaleDateString('id-ID') : new Date(p.created_at).toLocaleDateString('id-ID'); const cat = rData.catatan || '-'; const namaKaderAsli = window.getKaderName(p.username);
                ...
                return `<tr style="border-bottom:1px solid #eee;"><td><b style="color:#B8860B;">${tgl}</b></td><td><code style="color:var(--th-dark); font-weight:bold;">${p.id_sasaran_ref}</code></td><td>${namaKaderAsli}</td><td style="font-size:0.85rem; color:#444;">${cat}</td><td>${gpsStr}</td></tr>`;
            }).join('');
        };

        document.getElementById('flt-pend-nama').addEventListener('input', renderTabelPend); renderTabelPend();
        const btnKec = document.getElementById('dash-flt-kec'); if (btnKec) btnKec.addEventListener('change', () => { window.currentFilterKec = btnKec.value; window.currentFilterDesa = 'ALL'; window.renderAdminView('pendampingan'); });
        const btnDesa = document.getElementById('dash-flt-desa'); if (btnDesa) btnDesa.addEventListener('change', () => { window.currentFilterDesa = btnDesa.value; window.renderAdminView('pendampingan'); });
    }

    else if (target === 'cetak_laporan') {
        const blnNames = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']; const curDate = new Date(); const curRealMonth = curDate.getMonth() + 1; const curRealYear = curDate.getFullYear();
        let maxMonth = parseInt(window.currentFilterTahun) === curRealYear ? curRealMonth : 12; if(parseInt(window.currentFilterTahun) > curRealYear) maxMonth = 12; 
        let optBulan = ''; for(let i=1; i<=maxMonth; i++) { optBulan += `<option value="${i}" ${window.currentFilterBulan == i ? 'selected' : ''}>${blnNames[i-1]}</option>`; }
        const years = [2026, 2027, 2028, 2029, 2030]; let optTahun = years.map(y => `<option value="${y}" ${window.currentFilterTahun == y ? 'selected' : ''}>${y}</option>`).join('');

        content.innerHTML = `
            <style>
                @media print { body * { visibility: hidden; } #admin-sidebar, .admin-header, #filter-area-cetak { display: none !important; } #report-print-area, #report-print-area * { visibility: visible; color: black !important; } #report-print-area { position: absolute; left: 0; top: 0; width: 100%; padding: 10px; background: white; } #report-print-area table { width: 100%; border-collapse: collapse; } #report-print-area th, #report-print-area td { border: 1px solid black !important; } }
            </style>
            <div class="animate-fade">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; flex-wrap:wrap; gap:10px;">
                ...
                <div style="background:white; padding:20px; border-radius:8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); border: 1px solid #e1e8ed; overflow-x:auto;">
                    <div id="report-print-area" style="min-width: 800px;">
                        ...
                        <table style="width: 100%; margin-bottom: 20px; border: none; font-size: 0.9rem; color:black;"><tr><td style="width: 120px; border: none !important; font-weight:bold; padding:2px;">DESA</td><td style="border: none !important; font-weight:bold; padding:2px;">: <span id="r-desa"></span></td></tr><tr><td style="border: none !important; font-weight:bold; padding:2px;">KECAMATAN</td><td style="border: none !important; font-weight:bold; padding:2px;">: <span id="r-kec"></span></td></tr><tr><td style="border: none !important; font-weight:bold; padding:2px;">BULAN</td><td style="border: none !important; font-weight:bold; padding:2px;">: <span id="r-bulan"></span></td></tr></table>
                        <table id="tabel-rekap-kader" style="width: 100%; border-collapse: collapse; font-size: 0.85rem; text-align: center; color:black; margin-bottom: 30px;" border="1"><thead><tr><th rowspan="2" style="border: 1px solid black; padding: 8px;">NO</th><th rowspan="2" style="border: 1px solid black; padding: 8px;">NAMA KADER</th><th colspan="4" style="border: 1px solid black; padding: 8px;">JUMLAH SASARAN</th><th colspan="4" style="border: 1px solid black; padding: 8px;">JUMLAH PENDAMPINGAN</th><th rowspan="2" style="border: 1px solid black; padding: 8px;">TOTAL JUMLAH<br>PENDAMPINGAN</th></tr><tr><th style="border: 1px solid black; padding: 5px;">CATIN</th><th style="border: 1px solid black; padding: 5px;">BUMIL</th><th style="border: 1px solid black; padding: 5px;">BUFAS</th><th style="border: 1px solid black; padding: 5px;">BADUTA</th><th style="border: 1px solid black; padding: 5px;">CATIN</th><th style="border: 1px solid black; padding: 5px;">BUMIL</th><th style="border: 1px solid black; padding: 5px;">BUFAS</th><th style="border: 1px solid black; padding: 5px;">BADUTA</th></tr></thead><tbody id="r-tbody"></tbody></table>
                        <div style="display:flex; justify-content:flex-end; padding-right:30px; text-align:center; font-size:0.9rem; color:black;"><div style="width: 250px;"><div id="r-tgl-ttd" style="margin-bottom: 5px;">Singaraja, </div><div style="margin-bottom: 60px;">Mengetahui,<br>PKB Pengampu</div><div id="r-nama-pkb" style="font-weight:bold; text-decoration:underline; white-space: nowrap; overflow: visible;">(Nama PKB)</div><div id="r-nip-pkb" style="white-space: nowrap;">NIP. .........................</div></div></div>
                    </div>
                </div>
            </div>
        `;
        ...
        const renderReport = async () => {
            ...
            let pendTarget = filteredPend.filter(p => { let rData = {}; try { rData = JSON.parse(p.data_laporan || '{}'); } catch(e){} const dt = rData.tgl_kunjungan ? new Date(rData.tgl_kunjungan) : new Date(p.created_at); return dt.getMonth() + 1 === m && dt.getFullYear() === y; });
            const kaderMap = {};
        ...
        const btnKec = document.getElementById('dash-flt-kec'); if (btnKec) btnKec.addEventListener('change', () => { window.currentFilterKec = btnKec.value; window.currentFilterDesa = 'ALL'; window.renderAdminView('cetak_laporan'); });
        const btnDesa = document.getElementById('dash-flt-desa'); if (btnDesa) btnDesa.addEventListener('change', () => { window.currentFilterDesa = btnDesa.value; window.renderAdminView('cetak_laporan'); }); 
        renderReport();
    }

    else {
        content.innerHTML = `
            <div class="animate-fade" style="text-align:center; padding: 50px;">
                <div style="font-size: 4rem; margin-bottom: 15px;">🚧</div>
                <h2 style="color: var(--th-dark); margin-bottom: 10px;">Fitur Sedang Dibangun</h2>
                <p style="color: #666;">Halaman untuk <b>${target}</b> saat ini belum tersedia di panel Supervisor.</p>
            </div>
        `;
    }
};
...
export const initAdmin = async (session) => {
    ...
    document.body.innerHTML = `
        <div id="admin-root" style="position:absolute; top:0; left:0; right:0; bottom:0; display:flex; background:#eef2f5; font-family: 'Segoe UI', sans-serif; overflow: hidden;">
            ...
            <div id="admin-sidebar" style="width:260px; background: linear-gradient(180deg, var(--th-dark) 0%, var(--th-main) 100%); color:var(--th-text); display:flex; flex-direction:column; box-shadow: 2px 0 10px rgba(0,0,0,0.15); flex-shrink: 0;">
                ...
                <div style="flex:1; padding: 10px 0; overflow-y:auto;" id="admin-menu-container">
                    <div style="text-align:center; padding:20px; opacity:0.7;">⏳ Menyusun menu...</div>
                </div>
            ...
            <div style="flex:1; display:flex; flex-direction:column; overflow:hidden; width:100%;">
                <div class="admin-header" style="background:white; padding: 15px 25px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); z-index:5; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #e1e8ed;">
                    <div style="display:flex; align-items:center;"><button id="btn-mobile-menu">☰</button><h2 id="admin-page-title" style="margin:0; font-size:1.3rem; color:var(--th-dark); font-weight:800;">Memuat Data...</h2></div>
    ...
    const toggleMenu = () => { sidebar.classList.toggle('active'); backdrop.classList.toggle('active'); };
    btnMenu.addEventListener('click', toggleMenu); backdrop.addEventListener('click', toggleMenu);
                ...
                if (activeTarget === 'reload_app') {
                    if (confirm("🔄 TARIK PEMBARUAN SISTEM?\n\nPerintah ini akan membersihkan memori sistem (Cache) dan memuat ulang aplikasi ke versi terbaru.\n\nLanjutkan?")) {
                        try {
                            ...
                            alert("✅ Memori sistem berhasil dibersihkan! Memuat versi terbaru..."); window.location.reload(true);
                        } catch (e) { window.location.reload(true); }
                    }
                    return; 
                }
                document.querySelectorAll('.admin-menu-item, .sub-menu-item').forEach(m => m.classList.remove('active')); 
                item.classList.add('active'); 
                document.getElementById('admin-page-title').innerText = item.innerText.replace('▼', '').replace(/[^\w\s]/gi, '').trim(); 
    ...
    const buildDynamicMenus = () => {
        try {
            ...
            let filteredMenu = allMenu.filter(m => { 
                const roles = String(m.role_akses || '').toUpperCase(); 
                const isActive = String(m.is_active || 'Y').toUpperCase() === 'Y'; 
                return isActive && roles.includes(roleUpper); 
            }).sort((a,b) => (parseInt(a.urutan)||0) - (parseInt(b.urutan)||0));

            if (filteredMenu.length === 0) {
                filteredMenu = [
                    { id_menu: 'M1', label_menu: 'Dashboard', icon: '📊', target_view: 'dashboard' },
                    { id_menu: 'M2', label_menu: 'Database Sasaran', icon: '📋', target_view: 'sasaran' },
                    { id_menu: 'M3', label_menu: 'Riwayat Pendampingan', icon: '🤝', target_view: 'pendampingan' },
                    { id_menu: 'M4', label_menu: 'Database Kader TPK', icon: '👥', target_view: 'database_tpk' },
                    { id_menu: 'M5', label_menu: 'Cetak Laporan', icon: '🖨️', target_view: 'cetak_laporan' },
                    { id_menu: 'M6', label_menu: 'Update Sistem', icon: '🔄', target_view: 'reload_app' }
                ];
            }
                ...
                if(myChildren.length > 0) {
                    menuHtml += `
                        <div style="margin-bottom: 2px;">
                            <a class="admin-menu-item" style="display:flex; justify-content:space-between; align-items:center;" onclick="var c = this.nextElementSibling; if(c.style.display==='none'){c.style.display='block'; this.style.background='rgba(255,255,255,0.05)';}else{c.style.display='none'; this.style.background='';}">
                                <span>${p.icon || '📁'} ${p.label_menu}</span><span style="font-size:0.7rem; opacity:0.6;">▼</span>
                            </a>
                            <div style="display:none; background: rgba(0,0,0,0.15); border-left: 3px solid var(--th-accent);">
                                ${myChildren.map(c => `<div class="sub-menu-item" data-target="${c.target_view}">${c.icon || '📄'} ${c.label_menu}</div>`).join('')}
                            </div>
                        </div>`;
                } 
                else {
    ...
    document.getElementById('btn-admin-refresh').onclick = async () => {
        ...
        if(success) {
            buildDynamicMenus(); 
            const activeMenuEl = document.querySelector('.admin-menu-item.active, .sub-menu-item.active');
            if(activeMenuEl) window.renderAdminView(activeMenuEl.getAttribute('data-target'));
            else window.renderAdminView('dashboard');
        } else { alert("Gagal menyegarkan data. Periksa koneksi internet."); }
    ...
    if(success) {
        ...
        const mDash = document.querySelector('.admin-menu-item[data-target="dashboard"]');
        if(mDash) mDash.classList.add('active');
    } else {
